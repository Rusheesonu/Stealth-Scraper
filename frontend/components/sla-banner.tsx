...
export function SLABanner({ variant = "hero", className }: Props) {
  if (variant === "hero") {
    return (
      <div
        className={cn(
          "inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/5 px-4 py-1.5 text-xs font-mono text-emerald-700 dark:text-emerald-300",
          className,
        )}
      >
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-1.5">
          <h3 className="font-semibold tracking-tight text-[var(--color-fg)]">
            Reliability SLA — failed scrapes don&apos;t cost a credit.
          </h3>
          <p className="text-sm leading-relaxed text-[var(--color-fg-subdued)]">
            If a scrape hits an anti-bot wall, returns empty, or errors out —
            we auto-refund the credit within minutes. View your refund
            history at any time in <span className="font-mono">Settings → Refunds</span>.
