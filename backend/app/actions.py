...
async def run_actions(tab, actions: list[BrowserAction]) -> None:
    """Run actions sequentially. Each errors are surfaced but don't stop
    the chain — actions are best-effort. If a step is fundamental (e.g.
    you need to be logged in for the page to render), the subsequent
    snapshot/extract will fail naturally."""
    for i, a in enumerate(actions):
        ...
        try:
            ...
            print(f"[actions] step {i} ({kind}) failed: {e!r}")
